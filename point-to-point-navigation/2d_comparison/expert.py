...
class batch_crowd_nav():
    ...
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_traj_guess(self, initial_state, x_obs_trajectory, y_obs_trajectory, v_des, x_waypoint, y_waypoint, arc_vec, x_diff, y_diff, x_fin ,y_fin):
    ...
    def compute_projection_sampling(self, key, sol_x_bar, sol_y_bar, x_obs_trajectory_proj, y_obs_trajectory_proj, lamda_x, lamda_y, x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, initial_state, x_fin, y_fin):
        ...
        return c_x, c_y, x, xdot, xddot, y, ydot, yddot, res_norm_batch#, idx_ellite_projection
    ...
    def compute_cost_batch(self, x, y, xdot, ydot, xddot, yddot, x_project, y_project, res_norm_batch, x_fin, y_fin,  x_obs_trajectory, y_obs_trajectory):
        ...
        #cost_fin = jnp.linalg.norm(x-x_fin, axis = 1)+jnp.linalg.norm(y-y_fin, axis = 1)
        
        #cost_velocity = jnp.linalg.norm(  jnp.sqrt( xdot**2 + ydot**2) - self.v_des , axis = 1 )
        cost_batch = 1.0* res_norm_batch + 0.25*cost_smoothness + 0.2* cost_track + 4.0*cost_obs #+ 1.0*clearance_cost#+ 0.1 * cost_velocity#+ 0.1* cost_fin 
    ...
    def compute_shifted_samples(self, key, cost_batch, c_x_ellite, c_y_ellite, x_obs_trajectory, y_obs_trajectory,i, c_mean_prev, c_cov_prev):
        ...
        #d = jnp.exp((self.beta) * (d) )
        sum_d= jnp.sum(d, axis = 0) 
        c_mean = (1- self.alpha) * c_mean_prev + self.alpha * jnp.sum( (c_ellite * d[:,jnp.newaxis]) , axis= 0)/ sum_d

        #cost_batch_minus = cost_batch
        #d = (jnp.exp( self.beta * cost_batch_minus))
        #sum_d = -jnp.sum(d, axis = 0) 

        #c_mean =  jnp.sum( (d[:,jnp.newaxis] * c_ellite) , axis= 0)/ sum_d  #shape: num_batch
        ...
        #jnp.linalg.norm(jnp.maximum(jnp.zeros(( self.initial_up_sampling*self.num_batch, self.num*self.num_obs  )), dist_obs), axis = 1)
    ...
    def compute_controls(self, c_x_best, c_y_best, delta_t):
        ...
        ax_control = (xdot_best[self.num_sample] - xdot_best[0]) /  ((self.num_sample-1)*0.1) #jnp.mean(xddot_best[0:self.num_sample])
        ay_control = (ydot_best[self.num_sample] - ydot_best[0]) /  ((self.num_sample-1)*0.1)#jnp.mean(yddot_best[0:self.num_sample])
