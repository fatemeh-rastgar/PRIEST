...
class Optim():
    ...
    def __init__(self, way_point_shape, max_proj, num_goal, v_max, a_max, weight_smoothness,  nvar, num, num_obs, num_batch, a_obs, b_obs, c_obs, t_fin, vx_obs, vy_obs, vz_obs, vx_des, vy_des, vz_des ):
    ...
    def compute_x(self, x_obs_trajectory, y_obs_trajectory, z_obs_trajectory, d_obs, alpha_obs, beta_obs, alpha_a,  beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z,  b_eq_x, b_eq_y, b_eq_z ):
                     # b_projection_x = sol_x_bar
        # b_projection_y = sol_y_bar

        temp_x_obs = d_obs*jnp.cos(alpha_obs)*jnp.sin(beta_obs)*self.a_obs
        b_obs_x = x_obs_trajectory.reshape(self.num*(self.num_obs))+temp_x_obs
            
        temp_y_obs = d_obs*jnp.sin(alpha_obs)*jnp.sin(beta_obs)*self.b_obs
        b_obs_y = y_obs_trajectory.reshape(self.num*(self.num_obs))+temp_y_obs

        temp_z_obs = d_obs*jnp.cos(beta_obs)*self.c_obs
        b_obs_z = z_obs_trajectory.reshape(self.num*(self.num_obs))+temp_z_obs
        ...
        lincost_z =  -lamda_z-self.rho_obs*jnp.dot(self.A_obs.T, b_obs_z.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, b_az_ineq.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, b_vz_ineq.T).T
        ...
        sol_z = jnp.dot(cost_mat_inv, jnp.hstack(( -lincost_z, b_eq_z )).T).T
        ...
        return primal_sol_x, primal_sol_y, primal_sol_z, x, y, z, xdot, ydot, zdot, xddot, yddot, zddot



    ###############################################
    @partial(jit, static_argnums=(0,))	
    def initial_alpha_d(self, x_guess, y_guess, z_guess, xdot_guess, ydot_guess, zdot_guess, xddot_guess, yddot_guess, zddot_guess, x_obs_trajectory, y_obs_trajectory, z_obs_trajectory, lamda_x, lamda_y, lamda_z):
        ...
        wc_beta_temp = (z_guess-z_obs_trajectory[:,jnp.newaxis])
        wc_beta = wc_beta_temp.transpose(1, 0, 2)
        wc_beta = wc_beta.reshape(self.num_goal, self.num*(self.num_obs))
        ws_beta = wc_alpha / jnp.cos(alpha_obs)

        beta_obs = jnp.arctan2( ws_beta*self.c_obs, wc_beta*self.a_obs )

        c1_d = 1.0*self.rho_obs* self.a_obs**2*jnp.sin(beta_obs)**2 + 1.0*self.rho_obs * self.c_obs**2*jnp.cos(beta_obs)**2
        c2_d = 1.0*(self.a_obs*self.rho_obs*wc_alpha*jnp.cos(alpha_obs)*jnp.sin(beta_obs) + self.b_obs*self.rho_obs*ws_alpha*jnp.sin(alpha_obs)*jnp.sin(beta_obs) + self.c_obs*self.rho_obs*wc_beta*jnp.cos(beta_obs))
        ...
        res_x_obs_vec = wc_alpha-self.a_obs*d_obs*jnp.cos(alpha_obs)*jnp.sin(beta_obs)
        res_y_obs_vec = ws_alpha-self.b_obs*d_obs*jnp.sin(alpha_obs)*jnp.sin(beta_obs)
        res_z_obs_vec = wc_beta-self.c_obs*d_obs*jnp.cos(beta_obs)


        lamda_x = lamda_x -self.rho_obs*jnp.dot(self.A_obs.T, res_x_obs_vec.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, res_ax_vec.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, res_vx_vec.T).T# -self.rho_f*jnp.dot(self.A_f.T, res_fx_vec.T).T
        lamda_y = lamda_y -self.rho_obs*jnp.dot(self.A_obs.T, res_y_obs_vec.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, res_ay_vec.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, res_vy_vec.T).T# -self.rho_f*jnp.dot(self.A_f.T, res_fy_vec.T).T
        lamda_z = lamda_z -self.rho_obs*jnp.dot(self.A_obs.T, res_z_obs_vec.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, res_az_vec.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, res_vz_vec.T).T# -self.rho_f*jnp.dot(self.A_f.T, res_fz_vec.T).T
    ...
    @partial(jit, static_argnums=(0,))	
    def compute_alph_d_proj(self, x, y, z, xdot, ydot, zdot, xddot, yddot, zddot, x_obs_trajectory, y_obs_trajectory, z_obs_trajectory, lamda_x, lamda_y, lamda_z):
        ...
        wc_beta_temp = (z-z_obs_trajectory[:,jnp.newaxis])
        wc_beta = wc_beta_temp.transpose(1, 0, 2)
        wc_beta = wc_beta.reshape(self.num_goal, self.num*(self.num_obs))
        ws_beta = wc_alpha / jnp.cos(alpha_obs)
    ...
    ##############################################
    @partial(jit, static_argnums = (0,) )
    def compute_cost_batch(self, x, y, z, xddot, yddot, zddot, x_obs, y_obs, z_obs, res_norm_batch , x_project, y_project, z_project):

        dist_x_temp = (x-x_obs[:,jnp.newaxis])
        dist_y_temp = (y-y_obs[:,jnp.newaxis])
        dist_z_temp = (z-z_obs[:,jnp.newaxis])
        
        dist_x = dist_x_temp.transpose(1, 0, 2)#[:, :, 0:30]
        dist_y = dist_y_temp.transpose(1, 0, 2)#[:, :, 0:30]
        dist_z = dist_z_temp.transpose(1, 0, 2)

        dist_x = dist_x.reshape(self.num_goal, self.num*self.num_obs)
        dist_y = dist_y.reshape(self.num_goal, self.num*self.num_obs)
        dist_z = dist_z.reshape(self.num_goal, self.num*self.num_obs)

        dist_obs = -dist_x**2/(self.a_obs**2)-dist_y**2/(self.b_obs**2)-(dist_z**2)/(self.c_obs**2)+1
        ...
        cost_track = jnp.linalg.norm(x-x_project, axis = 1)+jnp.linalg.norm(y-y_project, axis = 1)+4*jnp.linalg.norm(z-z_project, axis = 1)
        clearance_cost = - jnp.min(dist_obs, axis = 1)
        
        cost_max_x = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (x-self.x_max) ), axis = 1)#+jnp.linalg.norm(y-y_project, axis = 1)+2*jnp.linalg.norm(z-z_project, axis = 1) 
        cost_max_y = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (y-self.y_max) ), axis = 1)
        cost_max_z = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (z-self.z_max) ), axis = 1)
        cost_max = cost_max_x + cost_max_y + cost_max_z

        cost_min_x = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (-x+self.x_min) ), axis = 1)#+jnp.linalg.norm(y-y_project, axis = 1)+2*jnp.linalg.norm(z-z_project, axis = 1) 
        cost_min_y = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (-y+self.y_min) ), axis = 1)
        cost_min_z = jnp.linalg.norm( jnp.maximum( jnp.zeros((self.num_goal, self.num)),  (-z+self.z_min) ), axis = 1)
        cost_min = cost_min_x + cost_min_y + cost_min_z

        
        
        cost_batch = res_norm_batch + 10.0*cost_obs + 0.02*cost_smoothness + 0.2*cost_track+cost_min+cost_max #+ clearance_cost
    ...
    @partial( jit, static_argnums = (0,) )
    def SolveOpt1(self, x_waypoint,  y_waypoint, z_waypoint, arc_vec, x_samples, y_samples, z_samples, xdot_samples, ydot_samples, zdot_samples, xddot_samples, yddot_samples, zddot_samples, x_obs, y_obs, z_obs,  x_des_traj_init, y_des_traj_init, z_des_traj_init, initial_state, x_fin, y_fin, z_fin, lamda_x, lamda_y, lamda_z):
        
        b_eq_x, b_eq_y, b_eq_z = self.compute_boundary_vec(initial_state, x_fin, y_fin, z_fin)
        alpha_obs_init, beta_obs_init, d_obs_init, alpha_a_init, beta_a_init, d_a_init, alpha_v_init, beta_v_init, d_v_init, lamda_x_init, lamda_y_init, lamda_z_init = self.initial_alpha_d( x_samples, y_samples, z_samples,  xdot_samples, ydot_samples, zdot_samples, xddot_samples, yddot_samples, zddot_samples,  x_obs, y_obs , z_obs, lamda_x, lamda_y, lamda_z)
        
        c_x_init = jnp.zeros((self.num_goal,self.nvar))
        c_y_init = jnp.zeros((self.num_goal,self.nvar))
        c_z_init = jnp.zeros((self.num_goal,self.nvar))

        x_init = jnp.zeros((self.num_goal,self.num))
        y_init = jnp.zeros((self.num_goal,self.num))
        z_init = jnp.zeros((self.num_goal,self.num))

        xdot_init = jnp.zeros((self.num_goal,self.num))
        ydot_init = jnp.zeros((self.num_goal,self.num))
        zdot_init = jnp.zeros((self.num_goal,self.num))

        xddot_init = jnp.zeros((self.num_goal,self.num))
        yddot_init = jnp.zeros((self.num_goal,self.num))
        zddot_init = jnp.zeros((self.num_goal,self.num))

        res_init = jnp.zeros(self.num_goal)

        def lax_projection(carry, proj_iter ):
        
            c_x, c_y, c_z, x, xdot, xddot, y, ydot, yddot, z, zdot, zddot, res_norm_batch, alpha_obs, beta_obs, d_obs, alpha_a, beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z = carry
            c_x, c_y, c_z, x, y, z, xdot, ydot, zdot, xddot, yddot, zddot = self.compute_x(x_obs, y_obs, z_obs, d_obs, alpha_obs, beta_obs, alpha_a, beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z, b_eq_x, b_eq_y, b_eq_z )
            alpha_obs, beta_obs, d_obs, alpha_a, beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z, res_norm_batch = self.compute_alph_d_proj( x, y, z, xdot, ydot, zdot, xddot, yddot, zddot, x_obs, y_obs, z_obs, lamda_x, lamda_y, lamda_z)

            return (c_x, c_y, c_z, x, xdot, xddot, y, ydot, yddot, z, zdot, zddot, res_norm_batch, alpha_obs, beta_obs, d_obs, alpha_a, beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z), x
           
            

        carry_init =   c_x_init, c_y_init, c_z_init, x_init, xdot_init, xddot_init, y_init, ydot_init, yddot_init, z_init, zdot_init, zddot_init, res_init, alpha_obs_init, beta_obs_init, d_obs_init, alpha_a_init, beta_a_init, d_a_init, alpha_v_init, beta_v_init, d_v_init, lamda_x_init, lamda_y_init, lamda_z_init
        carry_fin, result = lax.scan(lax_projection, carry_init, jnp.arange(self.maxitet_proj))
        
        c_x, c_y, c_z, x, xdot, xddot, y, ydot, yddot, z, zdot, zddot, res_norm_batch, alpha_obs, beta_obs, d_obs, alpha_a, beta_a, d_a, alpha_v, beta_v, d_v, lamda_x, lamda_y, lamda_z = carry_fin

        x_sample_flat = x_samples.reshape(self.num_goal* self.num)
        y_sample_flat = y_samples.reshape(self.num_goal* self.num)
        z_sample_flat = z_samples.reshape(self.num_goal* self.num)
        
        normal_distance, x_project, y_project, z_project = self.vectorized_projection(x_waypoint,  y_waypoint, z_waypoint, x_sample_flat , y_sample_flat, z_sample_flat, arc_vec)

        x_project = x_project.reshape(self.num_goal, self.num)
        y_project = y_project.reshape(self.num_goal, self.num)
        z_project = z_project.reshape(self.num_goal, self.num)
        
        cost_func  = self.compute_cost_batch(x, y, z, xddot, yddot, zddot, x_obs, y_obs, z_obs, res_norm_batch, x_project, y_project, z_project )
        ...
        sol_z_best = c_z[idx_min]

        
        return sol_x_best, sol_y_best, sol_z_best, x_best, y_best, z_best, x, y, z
